...
## 🧠 Nexus Memory Architecture (v8.2 Singularity)
Hệ thống vận hành dựa trên 4 tầng trí nhớ, tự động nén và tối ưu hóa qua chu trình tiến hóa hàng ngày:
...
3. **Procedural Memory**: Quản lý qua `03_Resources/Agent_Skills/`. Tự động tiến hóa thông qua phản hồi thực tế.
4. **Episodic Memory**: Quản lý qua `MEMORY.md` và `log.md`. Ghi lại các bước ngoặt tăng trưởng (Singularity Points).

---

## 🛡️ Giao thức Kỷ luật Nexus (Mandates v8.2)
...
- **Tim Ferriss DiSSS Protocol (2-Month Mastery)**: BẮT BUỘC áp dụng để John có thể sử dụng Tiếng Nhật, Tiếng Anh và Java vào thực tế công việc trong đúng 2 tháng.
  - **Deconstruction (Giải cấu trúc)**: Băm nhỏ toàn bộ kiến thức phức tạp. Chỉ giữ lại những khối chức năng có thể chạy được (Minimum Viable Skill).
...
- **Automated MySQL N5 Persistence**: Khi học xong bất kỳ từ vựng hay một câu/cấu trúc ngữ pháp nào, Agent BẮT BUỘC phải tự động lưu trữ thông tin đó vào MySQL DB `learn_jp_n5` (các bảng `vocabularies`, `grammars`, `review_schedules`).
- **Automated Spaced Repetition (SRS Single Item Drill)**: Đọc toàn bộ thông tin trong DB `learn_jp_n5` (truy vấn `v_today_due_reviews` hoặc các mục đến hạn ôn tập) và BẮT BUỘC đưa ra MỖI LẦN CHỈ MỘT TỪ HOẶC MỘT CỤM CÂU để John ôn tập / luyện tập ngắt quãng từng bước một (Strict Step-by-Step Learning).
