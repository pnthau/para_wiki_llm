...
| **Chú thích (Comment)**          | Bắt đầu bằng `/*` và kết thúc bằng `*/`.<br>Có thể viết trên 1 dòng hoặc nhiều dòng. Trình duyệt bỏ qua hoàn toàn.<br>```/* Đây là chú thích một dòng */ /*  Đây là chú thích  nhiều dòng */```                                                                                                                                                                                                                     |
| **3 Cách nhúng CSS vào HTML**    | **1. External (Tệp ngoài — KHUYẾN NGHỊ)**: Tách biệt hoàn toàn CSS sang file `.css`.<br>```<link rel="stylesheet" href="style.css">```<br>**2. Internal (Trong thẻ `<style>`)**: Viết CSS trong `<head>` của file HTML.<br>```html<br><style> p { color: red; } </style><br>```<br>**3. Inline (Trực tiếp trên thẻ)**: Ưu tiên cao nhất nhưng khó bảo trì.<br>```<p style="color: red;">Text</p>```                 |
